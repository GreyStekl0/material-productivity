...
public class NavigationState internal constructor(
    internal val startKey: NavKey,
    internal val topLevelStack: NavBackStack<NavKey>,
    internal val subStacks: Map<NavKey, NavBackStack<NavKey>>,
) {
    public val currentTopLevelKey: NavKey by derivedStateOf { topLevelStack.last() }

    public val topLevelKeys: Set<NavKey>
        get() = subStacks.keys
    ...
    public val currentKey: NavKey by derivedStateOf { currentSubStack.last() }
}
 ...
 */
@Composable
public fun NavigationState.toEntries(entryProvider: (NavKey) -> NavEntry<NavKey>): SnapshotStateList<NavEntry<NavKey>> {
